...
// 같은 memory space에 있으면 reference는 ---> 이걸 llm에게 물어봐야함!!!!!!!!!!!!!!
// 같은 ememory space에 있지 않으면 , reference는 pointer처럼 행동한다.
// professor 강조 : *,& is a operator and it always return outcome.

// compiler doesn't care abuout the name of variable. it only considers about memory address. but the reason type matters is because of the size of memory.
// so local variable transforms in compiler, starting address of stack pointer  + offset.
